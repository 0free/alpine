...
profile_linux() {
   ...
   kernel_flavors='lts'
   kernel_addons='zfs'
   kernel_cmdline='unionfs_size=2500M'
   apks="\$apks
...
sed -i 's|loop,ro|loop,rw|g' ~/aports/main/openrc/modloop.initd
sed -i 's|overlay_size=.*|overlay_size=2500M|' ~/aports/main/openrc/modloop.confd
